...
def compute_psnr(a, b):
    psnr = 0.
    for dim in range(a.size(0)):
        mse = torch.mean((a[dim:dim+1] - b[dim:dim+1])**2).item()
        psnr += -10 * math.log10(mse)
    return psnr/a.size(0)

def compute_msssim(a, b):
    msssim = 0.
    for dim in range(a.size(0)):
        msssim += ms_ssim(a[dim:dim+1], b[dim:dim+1], data_range=1.).item()
    return msssim/a.size(0)

################## split one dataset into seen/unseen frames ##################
...
def msssim_fn(output_list, target_list):
    ...
    return msssim
########## SSIM ############
def ssim_fn_single(output, gt):
    return ssim(output.float().detach(), gt.detach(), data_range=1, size_average=False).cpu()

def ssim_fn_batch(output_list, gt):
    ssim_list = [ssim_fn_single(output.detach(), gt.detach()) for output in output_list]
    return torch.stack(ssim_list, 0).cpu()

############## SAM ##############
def sam_fn_single(output, gt, eps=1e-8):
    """
    output, gt: [B, C, H, W] or [C, H, W]
    Returns SAM in radians per batch item
    """
    if output.dim() == 4:
        ...
        C, H, W = output.shape
        output = output.view(C, -1).unsqueeze(0)
        gt = gt.view(C, -1).unsqueeze(0)
    else:
        raise ValueError("Unsupported tensor shape")

    dot_product = (output * gt).sum(dim=1)
    output_norm = output.norm(dim=1)
    gt_norm = gt.norm(dim=1)

    cos_theta = dot_product / (output_norm * gt_norm + eps)
    cos_theta = torch.clamp(cos_theta, -1.0 + eps, 1.0 - eps)

    sam = torch.acos(cos_theta)  # [B, N]
    return sam.mean(dim=1)       # [B]

def sam_fn_batch(output_list, gt):
    sam_list = [sam_fn_single(output, gt) for output in output_list]
    return torch.stack(sam_list, 0).cpu()
